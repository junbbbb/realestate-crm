# 크롤링 운영 가이드 — BEST MOUNTAIN

네이버 부동산(`fin.land.naver.com`)에서 마포구 상업용 매물(7종)을 일별로 수집한다.
...
| `realEstateTypes` | `["D01","D02","D03","D04","D05","E01","Z00"]` | 사무실·상가·상가건물·건물·상가주택·오피스·기타/토지 (7종 전부) |
...
### sync-to-supabase.py 동작 순서

1. **JSON 로드** → `map_fin_article()`로 properties row 변환
2. **기존 DB 전체 로드** — 가격 비교를 위해 `id, price, warrant_price, monthly_rent` 조회 (1000건씩 페이지네이션)
3. **가격 변동 감지** — 각 매물의 현재 가격 vs DB 이전 가격 비교
   - 신규: `price_change="new"`, change_type="initial"
   - 변동: increase/decrease 판정, `prev_price` 기록
   - 동일: `price_change="none"`
4. **properties upsert** — 100건씩 배치 upsert
5. **price_history insert** — 신규/변동건만 기록
6. **7일 미확인 비활성화** — `last_seen_at < 7일 전` → `is_active=false`
7. **가격 변동 랭킹 계산** (`price_change_rankings` 테이블):
   - `prev_price >= 100만원` + `price > 0`인 상승/하락 매물 필터
   - 변동률 `(current - prev) / prev` 기준 정렬
   - 상승 TOP 6 + 하락 TOP 6 = 최대 12행
   - 기존 전체 삭제 후 재삽입 (`DELETE ... WHERE id != 0` → `INSERT`)
   - 대시보드에서 O(1) 조회 가능

### naver_detail_cache 정리 전략

`src/lib/naver-detail.ts`의 `setDbCache()` 호출 시 별도 정리 로직은 없으나:
- **인메모리 캐시**: Map 최대 200항목, FIFO 방식으로 오래된 것 제거 (5분 TTL)
- **DB 캐시**: `naver_detail_cache` 테이블, 24시간 TTL (조회 시 age 체크)
- **정리 방식**: 현재 DB 레벨에서 오래된 행 자동 삭제는 구현되어 있지 않음. 마이그레이션에 `fetched_at` 인덱스가 존재하여 향후 7일 초과 행 삭제 cron 또는 확률적 정리를 붙일 수 있음
- `PROJECT_STATUS.md`에 "5% 확률로 7일 초과 행 삭제"로 기록되어 있으나, 현재 코드에는 해당 확률적 정리 로직이 구현되어 있지 않음 (향후 추가 예정)
